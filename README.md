...
图工作流模式让 AI 按计划图推进需求、模块和原子任务，而不是自由调用工具。

- **计划阶段**：把目标拆成带依赖关系的模块与原子任务，并形成当前计划。
- **原子模式**：每个原子任务验证通过后暂停，只授权继续下一个任务。
- **模块模式**：连续完成已授权模块，在模块边界、决策点或失败时停下。
- **自动驾驶模式**：持续处理可构建任务，但仍尊重歧义、失败与用户暂停边界。
- **构建阶段**：按依赖顺序检查就绪性，生成并应用代码与测试工件。
- **验证阶段**：运行测试、类型检查和 lint；只有诊断通过的节点才能标记为已验证。
- **失败恢复**：诊断失败时保留证据，在有限修复预算内更正后重新验证。

> 详细的图工作流模式文档请参见 [`docs/graph-mode.md`](./docs/graph-mode.md)
