...
export class PlanConflict extends Schema.TaggedErrorClass<PlanConflict>()("GraphWorkflowState.PlanConflict", {
  expectedPlanHash: Schema.String,
  actualPlanHash: Schema.String,
}) {}
...
export interface Interface {
  ...
  readonly setMode: (input: {
    ...
    State,
    RevisionConflict | ActiveWorkflowError | ModuleScopeError | GraphStorage.SnapshotDecodeError
  >
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
          ...
          () =>
            Effect.gen(function* () {
              ...
              if (input.mode === "module" && graph) {
                yield* Effect.forEach(orderedAtomicNodes(graph), (node) => requireModule(graph, node.id), {
                  discard: true,
                })
              }
              ...
              const checkpointStatus = current?.currentNodeID
                ...
                : "none"
              const hardCheckpoint =
                current?.checkpointKind === "pause" ||
                current?.checkpointKind === "decision" ||
                current?.checkpointKind === "failure"
              const checkpointKind = current?.currentNodeID
                ? initialSelection
                  ? input.mode === "autopilot"
                    ? null
                    : input.mode
                  : hardCheckpoint || input.mode === "autopilot"
                    ? (current?.checkpointKind ?? null)
                    : input.mode
                : null
              const row = yield* database.db
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              ...
              const scope =
                current?.mode === "module" && currentNodeID
              ...
              const checkpointStatus =
                currentNodeID && current?.mode ? (current.mode === "autopilot" ? "none" : "approved") : "none"
              const row = yield* database.db
                .insert(GraphWorkflowStateTable)
                .values({
                  session_id: input.sessionID,
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              ...
              if (existing?.checkpointStatus === "approved" && existing.revision === input.expectedRevision + 1) {
    ...
    const advance = (current: State, nodeID: NodeID, graph: GraphView) =>
      Effect.gen(function* () {
        const ordered = orderedAtomicNodes(graph)
        const ready = ordered.filter((node) => isReady(graph, node.id))
        const next = current.mode === "module" ? yield* nextModuleTask(graph, nodeID, ready) : ready[0]
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              ...
              if (
                current.currentNodeID !== input.nodeID ||
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              ...
              return yield* advance(current, input.nodeID, yield* storage.currentPlan({ sessionID: input.sessionID }))
            }),
          { behavior: "immediate" },
        )
        .pipe(Effect.catchTag("SqlError", Effect.die))
    ...
    const recoverAbandonedArtifactApply = Effect.fn("GraphWorkflowState.recoverAbandonedArtifactApply")(function* (
      sessionID: string,
    ) {
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              ...
              const age =
                current.activeOperationStartedAt === null ? null : Math.max(0, now - current.activeOperationStartedAt)
              ...
              yield* recordTransition({
                state,
                toolName: "graph.artifact.apply.recovered",
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              const current = yield* get(input.sessionID)
              if (!current)
                return yield* new RevisionConflict({ expectedRevision: input.expectedRevision, actualRevision: 0 })
              if (current.activeOperationID === input.operationID && ownerIsCurrentRuntime(current)) return current
              if (current.activeOperationID)
                return yield* new ArtifactApplyOwnershipConflict({
              ...
              const row = yield* database.db
                ...
                .where(
                  and(
                    eq(GraphWorkflowStateTable.session_id, input.sessionID),
                    eq(GraphWorkflowStateTable.revision, current.revision),
                  ),
                )
                .returning()
              ...
              if (!row)
                return yield* new RevisionConflict({
                  expectedRevision: input.expectedRevision,
                  actualRevision: (yield* get(input.sessionID))?.revision ?? 0,
                })
              return fromRow(row)
            }),
          { behavior: "immediate" },
        ...
        .transaction(
          () =>
            Effect.gen(function* () {
              ...
              yield* database.db
                .update(GraphNodeTable)
                .set({ status: "implemented", test_status: "pending" })
                .where(eq(GraphNodeTable.id, input.nodeID))
                .run()
                .pipe(Effect.orDie)
              yield* audit.tool.record({
              ...
              const row = yield* database.db
                ...
                .where(
                  and(
                    eq(GraphWorkflowStateTable.session_id, input.sessionID),
                    eq(GraphWorkflowStateTable.revision, current.revision),
                    eq(GraphWorkflowStateTable.active_operation_id, input.operationID),
                  ),
                )
                .returning()
                .get()
              ...
              if (!row)
                return yield* new RevisionConflict({
                  expectedRevision: input.reservedRevision,
                  actualRevision: (yield* get(input.sessionID))?.revision ?? 0,
                })
              return fromRow(row)
            }),
    ...
    return Service.of({
      ...
      recoverAbandonedArtifactApply,
      assertArtifactApplyOwner,
      beginArtifactApply,
