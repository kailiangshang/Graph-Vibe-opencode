...
type CockpitWorkflow = {
  ...
  tasks: Task[]
  modules: Array<{ id: string; name: string; progress: { verified: number; total: number }; tasks: Task[] }>
}
...
export function GraphCockpit(props: {
  ...
  const [local, setLocal] = createStore({ mobileTab: "tasks" as "tasks" | "graph" | "details" })
  const selected = () =>
    ...
    props.workflow.tasks[0]
  const node = () => props.graph.nodes.find((item) => item.id === selected()?.id)
  const dependencies = () =>
    props.graph.edges
      .filter((edge) => edge.targetID === selected()?.id && edge.relation === "blocks")
      .map((edge) => props.graph.nodes.find((item) => item.id === edge.sourceID)?.name)
      .filter((name): name is string => !!name)
  const evidence = () => selected()?.latestEvidence
  const checkpoint = () => props.workflow.checkpoint.status === "pending"
    ...
    <section class="graph-cockpit flex h-full min-h-0 flex-col text-text-strong" aria-label="Graph workflow cockpit">
      <header class="graph-glass flex min-h-16 flex-wrap items-center gap-3 border-b px-4 py-2">
        ...
        <label class="flex items-center gap-2 text-sm">
          ...
          <select
            ...
            value={props.workflow.mode ?? "module"}
        ...
        <Show
          when={checkpoint()}
          fallback={
            <button class="graph-action secondary" disabled={props.actionPending} onClick={props.onPause}>
              Pause
            </button>
          }
        >
          <button class="graph-action primary" disabled={props.actionPending} onClick={props.onContinue}>
            Continue
          </button>
        </Show>
      </header>
      ...
      <div class="graph-grid min-h-0 flex-1">
          ...
          <For each={props.workflow.modules}>
            {(module) => (
              <section class="border-b border-border-weak-base px-2 py-3">
                ...
                <For each={props.workflow.tasks.filter((task) => task.moduleName === module.name)}>
                  {(task) => (
                    <button
                      class="graph-task mb-1 flex min-h-11 w-full items-start gap-2 rounded-lg px-2 py-2 text-left"
                      classList={{ current: task.current, selected: task.id === selected()?.id }}
        ...
        >
          <Show
            when={selected()}
            fallback={<div class="p-6 text-sm text-text-weak">Select a task to inspect its workflow details.</div>}
          >
            {(task) => (
              <div class="flex min-h-full flex-col">
                <div class="flex-1 space-y-5 p-5">
                  <div>
                    <div class="text-xs uppercase tracking-[0.14em] text-text-weak">
                      {task().moduleName ?? "Ungrouped"}
                    </div>
                    <h2 class="mt-1 text-lg font-semibold">{task().name}</h2>
                  ...
                  <InspectorSection title="Verification criteria">
                    <Show when={task().verification?.criteria.length} fallback={<p>Project checks only</p>}>
                      <ul class="space-y-1">
                        <For each={task().verification?.criteria}>{(criterion) => <li>□ {criterion}</li>}</For>
                      </ul>
                    </Show>
                  </InspectorSection>
                  ...
                  <Show when={task().testStatus === "failed"}>
                ...
                <footer class="graph-glass sticky bottom-0 flex gap-2 border-t p-3">
                  <Show when={task().id !== props.workflow.currentTask?.id}>
                    <button
