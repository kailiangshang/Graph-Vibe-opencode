...
test("Main derives its rail and inspector from visible graph nodes without plan controls", () => {
  ...
  expect([...root.querySelectorAll("button")].some((button) => button.textContent === "Continue")).toBe(false)
  expect([...root.querySelectorAll("button")].some((button) => button.textContent === "Pause")).toBe(false)
  expect([...root.querySelectorAll("button")].some((button) => button.textContent === "Publish to Main")).toBe(false)
  root.querySelector<HTMLButtonElement>(".graph-task")!.click()
