...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, ${Product.current().cli} does not support MCP authentication yet.`
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`${Product.current().cli} auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
