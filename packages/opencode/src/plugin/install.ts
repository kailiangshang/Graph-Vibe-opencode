...
export type PatchDeps = {
  ...
  files: (dir: string, name: "opencode" | "graph-vibe" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, Product.current() === Product.GraphVibe ? ".graph-vibe" : ".opencode")
}

function patchName(kind: Kind): "opencode" | "graph-vibe" | "tui" {
  if (kind === "server") return Product.current().config
  return "tui"
}
