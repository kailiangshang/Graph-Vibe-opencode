...
export interface Interface {
  ...
  readonly prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error | ProductMigration.Required>
  readonly loop: (input: LoopInput) => Effect.Effect<SessionV1.WithParts, ProductMigration.Required>
  readonly shell: (
    input: ShellInput,
  ) => Effect.Effect<SessionV1.WithParts, Session.BusyError | ProductMigration.Required>
  readonly command: (input: CommandInput) => Effect.Effect<SessionV1.WithParts, Image.Error | ProductMigration.Required>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error | ProductMigration.Required> =
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<SessionV1.WithParts, ProductMigration.Required> = Effect.fn(
      "SessionPrompt.run",
    ...
    const loop: (input: LoopInput) => Effect.Effect<SessionV1.WithParts, ProductMigration.Required> = Effect.fn(
      "SessionPrompt.loop",
    ...
    const shell: (
      input: ShellInput,
    ) => Effect.Effect<SessionV1.WithParts, Session.BusyError | ProductMigration.Required> = Effect.fn(
      "SessionPrompt.shell",
      ...
      return yield* state.startShell(
        input.sessionID,
        lastAssistant(input.sessionID),
        shellImpl(input, ready).pipe(Effect.orDie),
        ready,
      )
    })
